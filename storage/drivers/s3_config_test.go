...
func TestS3ConfigValidateOKWithEndpointCredsAndPublicURL(t *testing.T) {
	cfg := S3Config{
...
func TestS3ConfigValidateCredentialsMustPair(t *testing.T) {
	cfg := S3Config{Bucket: "uploads", Region: "us-east-1", AccessKey: "ak"}
	if err := cfg.Validate(); err == nil {
		t.Fatalf("Validate() expected error for AccessKey without SecretKey")
	}
}

func TestS3ConfigValidateInvalidEndpoint(t *testing.T) {
	cfg := S3Config{Bucket: "uploads", Region: "us-east-1", Endpoint: "not-a-url"}
	if err := cfg.Validate(); err == nil {
...
func TestS3ConfigValidateInvalidPublicURL(t *testing.T) {
	cfg := S3Config{Bucket: "uploads", Region: "us-east-1", PublicURL: "not-a-url"}
	if err := cfg.Validate(); err == nil {
