...
func TestUpload_WithOwnershipAndHook(t *testing.T) {
	mock := &MockDriver{}
	hookCalled := false
	handler := NewHTTPHandler(NewService(mock)).WithOnUploadHook(func(ctx context.Context, meta *FileMetadata, authCtx *authn.AuthContext) error {
		hookCalled = true
		if meta.Ownership["company_id"] != "comp-123" {
			t.Errorf("expected company_id comp-123 in hook, got %v", meta.Ownership["company_id"])
		}
		return nil
	})
	...
	if metadata.Ownership["company_id"] != "comp-123" {
		t.Errorf("expected ownership company_id comp-123, got %v", metadata.Ownership["company_id"])
	}
	if !hookCalled {
