...
Escaping also reaches the bodies of `{{ define }}` and `{{ block }}`, which `text/template` keeps as
...
| | |
|---|---|
| `{{ raw .fragment }}` | emitted unescaped |
| `{{ cdata .body }}` | wrapped in CDATA, splitting any `]]>` in the value across two sections |

> [!WARNING]
> **`raw` is a genuine hole, and the document check is not a backstop for it.** In element content a
> stray `&` is caught, but in attribute or element-name position the forged output is well-formed and
> passes every check:
>
> ```xml
> <Consignment ref="{{ raw .ref }}"/>
> ```
> with `ref` set to `" classified="SECRET` yields `<Consignment ref="" classified="SECRET"/>`.
>
> Use `raw` only for a value your own code produced. Note also that the opt-out is recognised only as
> a bare call: `{{ (raw .x) }}` in parentheses is escaped anyway — the safe direction, but a surprise.
