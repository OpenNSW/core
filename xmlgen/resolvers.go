...
// keywordFuncs are names text/template treats as keywords only while no
// function of that name exists: parse.(*Tree).startParse sets breakOK from
// !hasFunction("break"). Registering a resolver called break would quietly
// demote the keyword to an ordinary call, changing what every {{ break }} in
// every template already means.
...
func reservedName(name string) bool {
	if builtinFuncs[name] || keywordFuncs[name] {
		return true
	}
