...
func injectEscaping(t *parse.Tree) error {
	if t == nil {
		return nil
	}
	return walkList(t, t.Root)
}
...
func walkNode(t *parse.Tree, n parse.Node) error {
	...
	case *parse.TemplateNode:
		// {{ template "x" }} renders an associated template. xmlgen parses a
		// single template with no associates, so this can only fail at
		// execution; it emits nothing here.
		return nil
