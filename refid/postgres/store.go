...
// Package postgres provides a PostgreSQL-backed implementation of refid.Store
// using database/sql and the pgx stdlib driver directly — no ORM. Open a
// connection with sql.Open("pgx", dsn) (this package blank-imports
// github.com/jackc/pgx/v5/stdlib to register that driver name) and pass the
// resulting *sql.DB to New.
package postgres
...
// Option configures optional behavior for a PostgreSQL Store.
...
// store is a PostgreSQL-backed implementation of refid.Store. It uses a
...
// New returns a refid.Store backed by PostgreSQL. db must already be opened
// against the pgx driver (see the package doc). By default it targets the
...
CREATE TABLE IF NOT EXISTS %s (
	...
	if _, err := db.ExecContext(ctx, ddl); err != nil {
		return fmt.Errorf("refid/postgres: failed to create table %q: %w", cfg.tableName, err)
	}
	return nil
}
...
func (s *store) Next(ctx context.Context, scopeKey string, max int64) (int64, error) {
	...
	if err != nil {
		...
		return 0, fmt.Errorf("refid/postgres: sequence increment failed for scope %q: %w", scopeKey, err)
	}
	return counter, nil
}
